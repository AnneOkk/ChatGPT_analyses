...
fixed <- c(2.5, # intercept of the change score
          ...
          0.02 # group * quality effect
          ) 

rand <-  list(0.06, 0.52) # Random intercepts and slopes

res <- 0.72 # residual variance
```
...
library(simr)
library(future)
# diag_like_em_post = change score 
model <- makeLmer(diag_like_em_change ~ experience + autonomy + know_exp + attitude + quality*group + (1|id) + (1|quality), fixef=fixed, VarCorr=rand, sigma=res, data=data_red)
model

#model <- lme4::lmer(diag_like_em_post ~ experience + autonomy + know_exp + attitude + quality + group + (group|quality) + (1|id), data=data)
#model
```
...
```{r}
#| include: true
sim <- powerSim(model, nsim=100, test = fcompare(diag_like_em_post ~ experience + autonomy + know_exp + attitude + quality + group + (1|id) + (1|quality)))
...
sim_treat_subj <- powerSim(model_ext_subj, nsim=100, test = fcompare(diag_like_em_post ~ experience + autonomy + know_exp + attitude + quality + group + (1+group|quality) + (1|id) + (1|group:id)))
sim_treat_subj
```
...
```{r}
#| include: true
p_curve_treat <- powerCurve(model, nsim = 100, test=fcompare(diag_like_em_post ~ experience + autonomy + know_exp + attitude + quality + group + (1|id) + (1|quality)), within="case+group+time", breaks=c(30,50,70, 90 ))
plot(p_curve_treat)
```
